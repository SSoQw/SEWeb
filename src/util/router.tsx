...
export const routesNoPageLayout: ReadonlyArray<RouteProps & { path: string }> = [
  {
    path: "/login",
    element: <LoginForm />
  }, {
...
export const createRoutesWithPageLayout = (routes: ReadonlyArray<RouteProps>) => routes.map((r) => <Route {...r} key={r.path} element={<PageLayout>{r.element}</PageLayout>} />);
export const createRoutesNoPageLayout = (routes: ReadonlyArray<RouteProps>) => routes.map((r) => <Route {...r}  key={r.path}/>); 
